...
public class ImageViewTouch extends ImageView {
	...
	public void initImgPos() {
		...
		// @ min : ���蹂몄�����
		/*
		 * if (imageWidth > width || imageHeight > height) { float xratio =
		 * (float)width / (float)imageWidth; float yratio = (float)height /
		 * (float)imageHeight;
		 * 
		 * // Math.min fits the image to the shorter axis. (with letterboxes
		 * around) // Math.max fits the image th the longer axis. (with the
		 * other axis cropped) value[0] = value[4] = Math.max(xratio, yratio); }
		 */
		...
		if (width > height) {
			value[0] = value[4] = Math.max(yratio, yratio);
		}// end if

		if (width < height) {
			value[0] = value[4] = Math.max(xratio, xratio);
		}// end if
			// @ add min End
...
//
//			float xratio = (float) this.getWidth()
//					/ (float) d.getIntrinsicWidth();
//			float yratio = (float) this.getHeight()
//					/ (float) d.getIntrinsicHeight();
//
//			Log.i("width", String.valueOf(this.getWidth()));
//			Log.i("height", String.valueOf(this.getHeight()));
//			Log.i("imageWidth", String.valueOf(d.getIntrinsicWidth()));
//			Log.i("imageHeight", String.valueOf(d.getIntrinsicHeight()));
//			Log.i("xratio", String.valueOf(xratio));
//			Log.i("yratio", String.valueOf(yratio));
//			Log.i("xratio",
//					String.valueOf((int) (yratio * d.getIntrinsicWidth())));
//			Log.i("yratio",
//					String.valueOf((int) (xratio * d.getIntrinsicWidth())));
//			Log.i("scaleWidth",
//					String.valueOf((int) (d.getIntrinsicWidth() * values[0])));
//			Log.i("scaleHeight",
//					String.valueOf((int) (d.getIntrinsicHeight() * values[4])));
//
//			if ((int) (yratio * d.getIntrinsicWidth()) >= (int) (d
//					.getIntrinsicWidth() * values[0])) {
//				Toast.makeText(context, "setVisibility", Toast.LENGTH_SHORT).show();
//				touchView_02.setVisibility(0);
//			}// end if
		...
		case MotionEvent.ACTION_MOVE:
			if (mode == DRAG) {
				...
				matrix.postTranslate(event.getX() - start.x, event.getY()
						- start.y);
				this.scrollTo((int)(start.x - event.getX()),(int)(start.y - event.getY()));
			} else if (mode == ZOOM) {
